...
//  const meansPollRef = useRef(null);
...
//     dispatch(fetchRackClusterMean(effectiveCluster));
//   } else {
//     if (realDcId) dispatch(fetchRackClusterMeansByDc(realDcId));
//   }

//   // If polling not requested, stop here
//   if (!pollInterval) return;

//   // clear existing
//   if (meansPollRef.current) {
//     clearInterval(meansPollRef.current);
//     meansPollRef.current = null;
//   }

//   meansPollRef.current = setInterval(() => {
//     if (effectiveCluster) {
//       dispatch(fetchRackClusterMean(effectiveCluster));
//     } else {
//       if (realDcId) dispatch(fetchRackClusterMeansByDc(realDcId));
...
// const rackPollRef = useRef(null);

// useEffect(() => {
//   // clear previous interval
//   if (rackPollRef.current) {
//     clearInterval(rackPollRef.current);
//     rackPollRef.current = null;
//   }
...
//   rackPollRef.current = setInterval(() => {
//     dispatch(fetchRackById(selectedRackId));
//   }, pollInterval);
...
//     }
//   };
// }, [selectedRackId, pollInterval, dispatch]);
...
export default function DashboardRightPanel({
  ...
  // Resolve effective → real DC id (role-aware)
  const getEffectiveDataCenterId = (maybeId) => {
    if (!maybeId) return null;
  ...
  // -------------------------
  // Polling: cluster means (immediate + interval)
  // -------------------------
  const meansPollRef = useRef(null);
  useEffect(() => {
    // nothing to fetch if no source
    if (!effectiveCluster && !effectiveDc) return;
  ...
  useEffect(() => {
    if (!meansLoading && !meanLoading) {
      if (effectiveCluster) {
        ...
        // use realDcId when marking DC context
        if (realDcId) dispatch(markContextFetched({ kind: "dc", id: realDcId }));
      }
    }
  }, [meansLoading, meanLoading, effectiveCluster, effectiveDc, realDcId, dispatch]);
  ...
  // -------------------------
  // Polling: selected rack details (immediate + interval)
  // -------------------------
  const rackPollRef = useRef(null);
  useEffect(() => {
    // clear previous interval
  ...
  // render means list
  const renderMeansList = () => {
    const listToShow = effectiveCluster ? (meanDetail ? [meanDetail] : []) : meansList || [];
