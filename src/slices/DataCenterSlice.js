...
// src/slices/DataCenterSlice.js
// add near top of file (inside module scope, above createSlice)
const getDcIdFromEntry = (entry) => {
  if (!entry) return null;
  // if top-level is a string id
  if (typeof entry === "string") return entry;
  // assignment shape: dataCenterId may be string or populated object
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateDataCenter.fulfilled, (state, action) => {
        ...
        if (updated && updated._id) {
          state.DataCenters = state.DataCenters.map((d) => {
            const entryId = getDcIdFromEntry(d);
            // match the real datacenter id (works for admin and assignment shapes)
            if (String(entryId) === String(updated._id)) {
              // if item is assignment-like, merge updated datacenter into dataCenterId
              if (d.dataCenterId) {
                // keep top-level assignment fields (like assignment _id) and update nested dc
                const nested = typeof d.dataCenterId === "object" ? { ...d.dataCenterId, ...updated } : { ...updated };
                return { ...d, dataCenterId: nested, name: nested.name ?? d.name };
              }
              // otherwise replace with updated datacenter object (admin case)
              return { ...updated };
            }
            return d;
          });
      ...
      .addCase(deleteDataCenter.fulfilled, (state, action) => {
        state.loading.delete = false;
        const removedId = action.payload;
        state.DataCenters = state.DataCenters.filter((d) => {
          // remove entries where either top-level _id matches (assignment entry id)
          // OR nested dataCenterId (string or object) matches the removed datacenter id
          if (String(d._id) === String(removedId)) return false;
          if (d.dataCenterId) {
            if (typeof d.dataCenterId === "string" && String(d.dataCenterId) === String(removedId)) return false;
            if (typeof d.dataCenterId === "object" && String(d.dataCenterId._id) === String(removedId)) return false;
          }
          return true;
        });
      })
