...
export default function AlertList({ title = "Alert", iconSrc = null, items = [] }) {
  ...
  return (
    <div className="w-full ">
      ...
      <div className="flex items-center justify-center mb-3 gap-2">
        {iconSrc && <img src={iconSrc} alt="Alert" className="w-6 h-6 rounded-full" />}
        <h3 className="font-semibold text-[#1E40AF] text-center">{title}</h3>
      </div>
                      ...
                      <div key={nestedItem.id} className="flex items-center justify-between py-0.5">
                        <div className="flex items-center">
                          ...
                          <span className="text-[#1E293B] text-sm font-medium">
                            {nestedItem.name}
                            {nestedItem.value !== undefined && nestedItem.value !== null ? (
                              <span className="text-[#64748B] text-sm ml-2"> — {String(nestedItem.value)}</span>
                            ) : null}
                          </span>
                        </div>
                        {nestedItem.date && (
