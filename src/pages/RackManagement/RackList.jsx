...
//   // restore simple display — do not filter/normalize IDs here
//   const displayRacks = Array.isArray(racks) ? racks : [];
...
//   // defensive row click (same as before)
//   const handleRowClick = (rack, e) => {
...
//                     item={rack}
//                     onEdit={(item) => {
//                       // simple edit behavior — no hub/sensor prefetch or defaults
//                       setEditingRack(item);
//                       setEditOpen(true);
//                     }}
//                     onDelete={(item) => {
...
// Updated files: RackList.jsx and RackEditModal.jsx
// Both updated to robustly extract nested ids (handles shapes like { id: { _id: '...' } })
// and avoid comparing objects to strings which caused the lists to disappear.
...
/**
 * Helper: extract nested id values safely
 */
const extractId = (v) => {
...
const RackList = ({ selectedRack: propSelectedRack, onRackSelect }) => {
  ...
  const displayRacks = Array.isArray(racks)
    ? racks.filter((r) => {
        ...
        const rackDcId = extractId(r.dataCenter) ?? extractId(r.dataCenter?.id) ?? extractId(r.dataCenterId);
        const rackHubId = extractId(r.hub) ?? extractId(r.hub?.id) ?? extractId(r.hubId);

        const selectedDcId = extractId(selectedDataCenter) ?? extractId(selectedDataCenter?.id) ?? null;
        const selectedHubId = extractId(selectedHub) ?? extractId(selectedHub?.id) ?? null;

        // debug helper (temporary): show resolved ids — remove after verification
        ...
        // if no DataCenter selected — follow your UI expectation: nothing to show
        if (!selectedDcId) return false;

        // if hub selected → only show racks for that hub
        if (selectedHubId) {
          return String(rackHubId) === String(selectedHubId);
        }

        return String(rackDcId) === String(selectedDcId);
      })
    : [];
  ...
  // open edit modal — keep simple behaviour (no defaults/prefetch here)
  const openEdit = (rackItem) => {
